@extends('layouts.app')
@section('title', 'Новое объявление || LBoard')
    ...
    <form action="{{ route('post.store') }}" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group mb-3">
            ...
            @foreach ($categories as $cat)
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="checkbox" id="cat_{{  $cat->id }}" name="category[{{ $cat->id }}]" value="{{ $cat->id }}">
        ...
        <div class="form-group mb-3">
            <input class="form-control @error('img_preview')
                is-invalid
            @enderror" type="file" name="img_preview" id="img_preview" value="{{ old('img_preview') }}">
            @error('img_preview')
                <span class="invalid-feedback">
                    <strong>{{ $message }}</strong>
                </span>
